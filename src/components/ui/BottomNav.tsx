 ...
 * fixed 80px height. The center FAB always routes to the `/play` mode picker;
 * the four side tabs go through the caller's `onTabPress` switch.
 */
        ...
        if (tab.center) {
          return (
            <Pressable
              key={tab.key}
              onPress={() => {
                if (activeTab !== 'play') router.push('/play');
              }}
